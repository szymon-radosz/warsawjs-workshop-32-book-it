import React, {
  useState,
  useCallback,
  useEffect,
  useMemo,
  memo,
  Suspense,
} from 'react';
import axios from 'axios';
import { Grid, Loader, Container } from 'semantic-ui-react';
import lazyWithPreload from '../../utils/lazyWithPreload';
import Filters from './Filters';
import SortBar from './SortBar';
import HotelsList from './HotelsList';
import ChartSwitcher from './ChartSwitcher';
import { ONLINE_URL, BEDS_TYPE } from '../../utils/const';
const RatingChart = React.lazy(() => import('./RatingChart'));

const SelectHotel = memo(props => {
  const [hotelData, setHotelData] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [bedsTypeFilter, setBedsTypeFilter] = useState({});
  const [hotelFilter, setHotelFilter] = useState('price');
  const [chartDisplay, setChartDisplay] = useState(false);

  useEffect(() => {
    const fetchData = () =>
      fetch(ONLINE_URL)
        .then(response => response.json())
        .then(data => {
          console.log(data.list);
          setHotelData(data.list.slice(0, 20));
          setIsLoading(false);
        });

    fetchData();
  }, []);

  const setFilteredBeds = useCallback(
    (key, value) => {
      setBedsTypeFilter({ ...bedsTypeFilter, [key]: value });
    },
    [bedsTypeFilter]
  );

  const filteredByBeds = useMemo(() => applyFilter(bedsTypeFilter, hotelData), [
    bedsTypeFilter,
    hotelData,
  ]);

  const filteredHotels = useMemo(() => applySort(filteredByBeds, hotelFilter), [
    filteredByBeds,
    hotelFilter,
  ]);

  return (
    <Container>
      <SortBar sortField={filteredHotels} setField={setHotelFilter} />
      <Layout>
        <Layout.Sidebar>
          <ChartSwitcher
            isChartVisible={chartDisplay}
            switchChartVisible={setChartDisplay}
          />
          <Filters
            count={useMemo(() => countHotelsByBedType(filteredHotels), [
              filteredHotels,
            ])}
            onChange={setFilteredBeds}
          />
        </Layout.Sidebar>
        <Layout.Feed isLoading={isLoading}>
          {chartDisplay && (
            <React.Suspense fallback={<Loader active inline="centered" />}>
              <RatingChart data={prepareChartData(filteredHotels)} />
            </React.Suspense>
          )}
          {isLoading ? (
            <Loader active inline="centered" />
          ) : (
            <HotelsList hotels={filteredByBeds} selectHotel={noop} />
          )}
        </Layout.Feed>
      </Layout>
    </Container>
  );
});

const noop = () => {};

function countHotelsByBedType(data) {
  console.log('countHotelsByBedType');
  return data.reduce(function(acc, v) {
    acc[v.room] = acc[v.room] ? acc[v.room] + 1 : 1;
    return acc;
  }, {});
}

function applyFilter(filters, data) {
  console.log('applyFilter');
  const isFilterSet = BEDS_TYPE.find(b => filters[b.value]);
  if (!isFilterSet) {
    return data;
  }
  const filtered = data.filter(h => filters[h.room]);
  return filtered;
}

function prepareChartData(hotels) {
  console.log('prepareChartData');
  return hotels.map(h => ({
    rating: +h.rating.average,
    price: +h.price.amount,
    reviews: +h.rating.reviews,
    name: h.title,
  }));
}
const sortHotels = {
  price: (a, b) => a.price.amount - b.price.amount,
  rating: (a, b) => b.rating.average - a.rating.average,
  reviews: (a, b) => b.rating.reviews - a.rating.reviews,
};

function applySort(hotels, sortField) {
  console.log('applySort');
  return hotels.sort(sortHotels[sortField]).concat([]);
}

const Layout = ({ children }) => (
  <Grid stackable divided>
    <Grid.Row>{children}</Grid.Row>
  </Grid>
);
const Sidebar = ({ children }) => (
  <Grid.Column width={4}>{children}</Grid.Column>
);

const Feed = ({ children }) => <Grid.Column width={12}>{children}</Grid.Column>;

Layout.Sidebar = Sidebar;
Layout.Feed = Feed;

export default SelectHotel;
